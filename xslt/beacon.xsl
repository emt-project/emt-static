...
<xsl:stylesheet
    ...
    <xsl:template match="/">#FORMAT: BEACON<xsl:text>&#xa;</xsl:text>#NAME:<xsl:value-of select="$project_title"/><xsl:for-each select=".//tei:person[.//tei:idno[@type='GND']/text()]"><xsl:text>&#xa;</xsl:text><xsl:value-of select=".//tei:idno[@type='GND']/text()"/>|<xsl:value-of select=".//tei:persName[@type='name']/text()"/>|<xsl:value-of select="$base_url"/><xsl:value-of select="data(@xml:id)"/>.html</xsl:for-each>
    </xsl:template>
</xsl:stylesheet>